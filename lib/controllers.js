...
function polymorphGetDocument(Model, isSingle, id) {
    var p = new MPromise;
    if (isSingle) {
        if (Model.formage && Model.formage.is_custom && Model.formage.custom_query.length > 0) {
            Model.findOne(Model.formage.custom_query[0], Model.formage.custom_query[1], Model.formage.custom_query[2], p.resolve.bind(p));
        } else {
    ...
    var router = {
            ...
            routes: {
                ...
                postLogin: function postLogin(req, res) {
                    registry.adapter.UsersModel.getByUsernamePassword(req.body.username, req.body.password, function (admin_user) {
                        ...
                        var dest = buildPath(req.session._loginReferrer || '');
            ...
            authMiddleware: function authMiddleware(role) {
                return function (req, res, next) {
                    ...
                    var admin_user = req.session && registry.adapter.getAdminUser(req.session._FormageUser);
                    if (!admin_user) {
